  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-5 md:p-6">
        <p className="font-mono text-[9px] sm:text-[10px] tracking-wider mb-1" style={{ color: project.accent }}>
          {project.tagline}
        </p>
        <h3 className="text-white font-semibold text-[14px] sm:text-lg mb-1.5 sm:mb-2">{project.title}</h3>
        <p className="text-gray-400 text-[11px] sm:text-sm leading-relaxed mb-3 sm:mb-4 line-clamp-3 flex-1">
          {project.description}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3 sm:mb-4">
          {project.tech.slice(0, 3).map((t) => (
            <span key={t} className="text-[9px] sm:text-[10px] font-medium text-gray-400 bg-white/[0.05] border border-white/[0.08] px-1.5 sm:px-2 py-0.5 rounded-full">
              {t}
            </span>
          ))}
          {project.tech.length > 3 && (
            <span className="text-[9px] sm:text-[10px] text-gray-500 px-1.5 py-0.5">
              +{project.tech.length - 3}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 text-[11px] sm:text-sm font-medium text-white border border-white/20 hover:border-white/40 rounded-lg py-1.5 sm:py-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ff2a2a]"
          >
...
export default function Projects() {
  ...
  return (
    <section
      id="projects"
      className="relative z-10 h-full flex flex-col pt-16 sm:pt-24"
    >
      {/* flex-1 min-h-0 — correctly fills remaining height inside a flex-col parent on desktop */}
      <div className="max-w-[1500px] mx-auto px-3 sm:px-8 lg:px-14 flex flex-col flex-1 min-h-0 w-full pb-4 sm:pb-6">

        {/* Heading — pinned, never scrolls */}
        <motion.div {...fadeUp()} className="mb-4 sm:mb-5 md:mb-7 flex-shrink-0">
          <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold text-white">Things I've built</h2>
        </motion.div>

        {/* Scrollable card grid — on desktop scrolls inside section; on mobile expands */}
        <div
          className="projects-grid-scroll flex-1 min-h-0 pb-4 pr-1"
          style={{ scrollbarWidth: 'thin', scrollbarColor: '#ff2a2a transparent' }}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-5 md:gap-6">
            {PROJECTS.map((p, i) => (
